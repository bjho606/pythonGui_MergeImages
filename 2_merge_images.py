...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", filetypes=(("PNG 파일", "*.png"),("모든 파일", "*.*")),\
        initialdir=r"C:\JB\Coding\Python\Practice\inflearn_lecture\resources")
...
def merge_image():
    ...
    # 최대 넓이, 전체 높이
    max_width, total_height = max(widths), sum(heights)     # 가장 큰 가로의 길이와 모든 세로 길이의 합 (이미지를 합칠 때 쓸 큰 도화지 개념)
    ...
    # progress bar 에 넣기 위해 수정
    for idx, img in enumerate(images):
        result_img.paste(img, (0, y_offset))
        y_offset += img.size[1]     # height 값 만큼 더해줌

        progress = (idx + 1) / len(images) * 100    # percent 정보를 계산
        p_var.set(progress)
        progressbar.update()
    
    dest_path = os.path.join(text_dest_path.get(), "merge_photo.jpg")
